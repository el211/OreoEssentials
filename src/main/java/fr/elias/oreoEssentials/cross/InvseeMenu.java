// File: src/main/java/fr/elias/oreoEssentials/cross/InvseeMenu.java
...
public final class InvseeMenu implements InventoryProvider {
    ...
    public void init(Player viewer, InventoryContents contents) {
        // staff can edit main inventory slots 0..35 (rows 0..3)
    ...
    public void refreshFromSession(InvseeSession sess) {
        Player viewer = null;
        // session is keyed by viewerId in service, but we only need the currently open inventory
        // SmartInvs will call update(...) on its own, so often this is enough and can even be empty.
        // If you want, you can force an update with inv.open(...) again, but it's usually not needed.
    }
