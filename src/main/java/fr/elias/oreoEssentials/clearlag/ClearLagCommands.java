...
public class ClearLagCommands implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "reload" -> {
                if (!sender.hasPermission("oreo.lag.reload")) { sender.sendMessage("§cNo permission."); return true; }
                mgr.reloadAndAck(sender);
            }
            case "clear" -> {
                if (!sender.hasPermission("oreo.lag.clear")) { sender.sendMessage("§cNo permission."); return true; }
                mgr.commandClear(sender);
            }
            case "killmobs" -> {
                if (!sender.hasPermission("oreo.lag.killmobs")) { sender.sendMessage("§cNo permission."); return true; }
